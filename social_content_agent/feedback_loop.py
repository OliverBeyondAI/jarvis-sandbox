"""Simulated feedback loop for iterative content refinement.
...
EVALUATE_TOOLS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "brand_voice_score": {
                    "type": "integer",
                    "description": "Score 1-10: How well does the post match the brand voice spec? (tone, personality, emoji usage, hashtag style, do/avoid lists)",
                },
                "engagement_score": {
                    "type": "integer",
                    "description": "Score 1-10: How likely is this post to drive engagement? (hook strength, CTA clarity, platform-native feel, shareability)",
                },
                "clarity_score": {
                    "type": "integer",
                    "description": "Score 1-10: How clear and compelling is the messaging? (value prop clarity, readability, visual description quality)",
                },
                "overall_score": {
                    "type": "integer",
                    "description": "Score 1-10: Overall quality assessment considering all factors",
                },
                "brand_voice_issues": {
                    "type": "array",
                    "items": {"type": "string"},
                ...
                "engagement_issues": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Specific engagement weaknesses (weak hook, missing CTA, poor platform fit, etc.)",
                },
                "strengths": {
                    "type": "array",
                    "items": {"type": "string"},
...
def _build_evaluate_system(product: ProductInfo, brand_voice: BrandVoice) -> str:
...
## Instructions
You will receive a set of social media posts. Use the `evaluate_post` tool ONCE for EACH post. Be specific in your feedback — vague feedback like "could be better" is not useful. Every suggestion must be concrete and actionable.
...
def _evaluate_posts(
    ...
    """Run the evaluator agent on a list of posts. Returns (feedback_list, input_tokens, output_tokens)."""
    ...
    for _ in range(10):  # safety limit
        ...
        for block in response.content:
            if block.type == "tool_use" and block.name == "evaluate_post":
                inp = block.input
                fb = PostFeedback(
                    day=inp.get("day", 0),
...
def _refine_posts(
    ...
    """Run the refiner agent on posts that need improvement. Returns (refined_posts, in_tokens, out_tokens)."""
    ...
    for _ in range(10):
        ...
        tool_results = []
        for block in response.content:
            if block.type == "tool_use" and block.name == "refine_post":
                inp = block.input
                post = SocialPost(
                    day=inp.get("day", 0),
