...
Frozen dataclass with environment variable defaults, brand voice presets,
...
BRAND_VOICE_PRESETS: dict[str, dict[str, Any]] = {
    "professional": {
        ...
        "style": (
            "Clear and direct. Use data-driven language. Avoid jargon unless "
    ...
    "startup": {
        ...
        "style": (
            "Short sentences. Punchy copy. Speak like a smart friend, not a "
    ...
    "luxury": {
        ...
        "style": (
            "Elegant prose with measured pacing. Evoke emotion and sensory "
    ...
    "technical": {
        ...
        "style": (
            "Lead with capabilities and specifications. Use structured formats "
...
@dataclass(frozen=True)
class Config:
    """Immutable configuration for the MCP Xena Marketing Agent."""
    ...
    # Extended thinking (key differentiator for Opus 4.7)
    thinking_budget: int = 10000  # default budget
    thinking_preset: str = "balanced"  # minimal, balanced, deep
    ...
    @property
    def resolved_thinking_budget(self) -> int:
        """Resolve thinking budget from preset or explicit value."""
        return self.THINKING_PRESETS.get(self.thinking_preset, self.thinking_budget)
    ...
    def brand_voice(self) -> dict[str, Any]:
        ...
        if self.brand_voice_custom:
            return {**base, **self.brand_voice_custom}
        return base
    ...
    def validate(self) -> list[str]:
        ...
        if not self.tavily_api_key:
            warnings.append("TAVILY_API_KEY not set — market research will use mock data")
        if not self.openai_api_key:
            warnings.append("OPENAI_API_KEY not set — image generation will be skipped")
